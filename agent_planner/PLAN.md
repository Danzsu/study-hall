...
### Completed
- Frontend P0 mojibake/copy issues on `Quiz`, `Written`, and `Glossary` are fixed in the current worktree.
- Quiz / Written / Glossary UX expansions are implemented.
- Study parser/render boundary hardening is implemented and reviewed.
- Blackbox smoke now covers API plus Home, Subject, Study, Search, Onboarding, and Glossary routes.
- Backend smoke-check script exists and passes against `localhost:3000` on live checks.
- `vercel.json` now sets the `validate-answer` timeout budget.

### In progress
- Content schema normalization beyond the Study render boundary.
- `lib/content.js` decomposition into smaller loader / normalizer / search units.
- Search and coverage improvements for the content pipeline.

### Next
- Finish the remaining frontend P1/P4 polish items that are still open.
- Tighten the canonical content schema and verifier coverage.
- Keep deploy readiness checks in the loop for each content run.
...
- [x] Ghost stack effekt (2 kártya halvány árnyék mögötte)
- [x] Result overlay flash (zöld/piros villanás swipe után)
- [x] Topic pill: colored glow border (subject.color alapján)

#### 🟡 P3 — Nice-to-have

**Glossary (`src/screens/Glossary.jsx`)**
- [x] Flash session módok: abbr-to-full, full-to-def, mixed
- [x] Concept cluster nézet (kategória csoportok, kártya rács)
- [x] Fogalomtérkép: dependency-free topic node map flash actionökkel

**Study — inline komponensek**
- [x] `<H>` highlight komponens (sárga kiemelés inline szövegben)
- [x] `<T>` tooltip komponens (hover → definíció popup)
