...
- Written Test, Quiz Results, and Glossary UX expansions are in place.
- Backend smoke-check is extended and a localhost:3000 live check passes.
- `vercel.json` now carries the `validate-answer` timeout budget.

### In progress
- Canonical content schema and `lib/content.js` refactor remain open.
- Study MDX/parser hardening remains the main user-facing risk.

### Next
- Keep the schema/coverage work and the parser refactor in the next backend batch.
- Finish the remaining P1/P2 UI polish only where it still matters.
