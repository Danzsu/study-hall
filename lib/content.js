...
const CACHE = new Map()
// The Python orchestrator writes content/ from a separate process, so the cache
// must expire on its own — clearCache() alone can't be reached cross-process.
const CACHE_TTL_MS = Number(process.env.CONTENT_CACHE_TTL_MS ?? 30_000)
...
function normalizeQuestion(q, idx = 0) {
  ...
  // Normalize fill_the_blanks: blank (object or array) → blanks (always array)
  let blanks
  if (q.blanks !== undefined) {
    blanks = Array.isArray(q.blanks) ? q.blanks : [q.blanks]
  } else if (q.blank !== undefined) {
    blanks = Array.isArray(q.blank) ? q.blank : [q.blank]
  }
  ...
  return {
    ...
    supervised: q.supervised ?? 'generated',
    image: q.image ?? null,
    ...
    // New type-specific fields (undefined when not applicable)
    answer: (type === 'true_false' || type === 'simple_input' || type === 'calc_input') ? (q.answer ?? '') : undefined,
    blanks: type === 'fill_the_blanks' ? (blanks ?? []) : undefined,
    choices: (type === 'drag_n_drop' || type === 'formula_drag_drop') ? (q.choices ?? []) : undefined,
    formulaText: type === 'formula_drag_drop' ? (q.formulaText ?? q.text ?? '') : undefined,
    formulaChips: type === 'calc_input' ? (q.formulaChips ?? q.formula_chips ?? []) : undefined,
  }
}
