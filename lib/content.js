...
export function getQuestions(slug) {
  return readJSON(path.join(CONTENT_DIR, slug, 'questions.json'), []).map(normalizeQuestion)
}

export function getFlashcards(slug) {
  return readJSON(path.join(CONTENT_DIR, slug, 'flashcards.json'), []).map((f, idx) => ({
    id: f.id ?? `f${idx + 1}`,
...
export function getGlossary(slug) {
  return readJSON(path.join(CONTENT_DIR, slug, 'glossary.json'), []).map((g, idx) => ({
    id: g.id ?? `g${idx + 1}`,
...
export function getDiagrams(slug) {
  return readJSON(path.join(CONTENT_DIR, slug, 'diagrams.json'), [])
}
